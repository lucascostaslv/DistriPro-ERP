...
const TransactionsManager = ({ products, onSaveTransaction, onUpdateTransaction, onDeleteTransaction, transactions, transactionCategories, setTransactionCategories, showNotification }) => {
  const [activeTab, setActiveTab] = useState('new');
  const [newCategoryName, setNewCategoryName] = useState('');

  const [type, setType] = useState('entry'); // entry | exit
  const [category, setCategory] = useState('Revenda');
  const [description, setDescription] = useState('');
  const [value, setValue] = useState('');
  const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
  
  // Controle de Itens (apenas para Revenda)
  ...
  const handleAddItem = () => {
    if (!selectedProduct || !qty || !unitCost) return showNotification('Preencha o produto, qtd e custo.', 'error');
    const prod = products.find(p => p.id === Number(selectedProduct));
    const mult = Number(conversionFactor) || 1;
    ...
    setItems([...items, { 
      ...
      cost: cost / mult, 
      total: quantity * cost,
      originalQty: quantity, // Para exibição
      conversionFactor: mult
    }]);
    setProductSearch('');
  ...
  // Calcula total automaticamente se for revenda
  const calculatedTotal = category === 'Revenda' ? items.reduce((acc, i) => acc + i.total, 0) : value;

  const handleSave = () => {
    if (!description || !date) return showNotification('Descrição e Data são obrigatórios.', 'error');
    if (category === 'Revenda' && items.length === 0) return showNotification('Adicione itens para movimentação de estoque.', 'error');
    if (category !== 'Revenda' && !value) return showNotification('Informe o valor.', 'error');
  ...
  const handleAddCategory = () => {
    if (!newCategoryName) return showNotification('Nome da categoria é obrigatório.', 'error');
    if (transactionCategories.find(c => c.name.toLowerCase() === newCategoryName.toLowerCase())) {
        return showNotification('Essa categoria já existe.', 'error');
    }
    setTransactionCategories([...transactionCategories, { id: Date.now(), name: newCategoryName, type: 'user' }]);
    setNewCategoryName('');
    showNotification('Nova categoria de movimentação adicionada!', 'success');
  };

  const handleDeleteCategory = (id) => {
      const category = transactionCategories.find(c => c.id === id);
      if (category && category.type === 'system') {
          return showNotification('Não é possível remover a categoria "Revenda".', 'error');
      }
      setTransactionCategories(transactionCategories.filter(c => c.id !== id));
  ...
  return (
    <div>
      <div className="flex gap-2 border-b border-slate-200 pb-2 mb-6">
        <button onClick={() => setActiveTab('new')} className={`px-4 py-2 text-sm font-medium rounded ${activeTab === 'new' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>Nova Movimentação</button>
        <button onClick={() => setActiveTab('categories')} className={`px-4 py-2 text-sm font-medium rounded ${activeTab === 'categories' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>Gerenciar Categorias</button>
      </div>

      {activeTab === 'new' && (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* Formulário */}
      <div className="lg:col-span-1 bg-white p-6 rounded border border-slate-200 shadow-sm h-fit">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-slate-800 flex items-center gap-2"><ClipboardList size={20}/> {editingId ? 'Editar Movimentação' : 'Nova Movimentação'}</h3>
          {editingId && <button onClick={handleCancelEdit} className="text-xs text-red-500 hover:underline">Cancelar</button>}
        </div>
        
        <div className="space-y-4">
          <div className="flex bg-slate-100 p-1 rounded">
            <button onClick={() => setType('entry')} className={`flex-1 py-2 text-sm font-bold rounded ${type === 'entry' ? 'bg-emerald-600 text-white shadow' : 'text-slate-600'}`}>Entrada (+)</button>
            <button onClick={() => setType('exit')} className={`flex-1 py-2 text-sm font-bold rounded ${type === 'exit' ? 'bg-red-600 text-white shadow' : 'text-slate-600'}`}>Saída (-)</button>
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1">Categoria</label>
            <select className="w-full border p-2 rounded text-sm" value={category} onChange={e => setCategory(e.target.value)} disabled={editingId && transactionCategories.find(c => c.name === category)?.type === 'system'}>
              {transactionCategories.map(cat => (
                <option key={cat.id} value={cat.name}>{cat.name}</option>
              ))}
            </select>
            {category === 'Revenda' && <p className="text-[10px] text-indigo-600 mt-1">* Atualiza o estoque automaticamente</p>}
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1">Descrição / Nº Nota</label>
            <input className="w-full border p-2 rounded text-sm" value={description} onChange={e => setDescription(e.target.value)} placeholder="Ex: NF 5504 ou Gasolina" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <input type="number" className="w-full border p-2 rounded text-sm bg-slate-50" value={calculatedTotal} onChange={e => category !== 'Revenda' && setValue(e.target.value)} readOnly={category === 'Revenda'} placeholder="0.00" />
            </div>
          </div>

          {category === 'Revenda' && (
            <div className="bg-slate-50 p-3 rounded border border-slate-200">
              <h4 className="text-xs font-bold text-slate-700 mb-2">Itens da Nota</h4>
              <div className="relative mb-2">
                ...
                {showProductSuggestions && productSearch && (
                  <div className="absolute z-10 w-full bg-white border border-slate-200 rounded shadow-lg max-h-40 overflow-y-auto mt-1">
                    {products.filter(p => p.name.toLowerCase().includes(productSearch.toLowerCase()) || (p.cbaCode && p.cbaCode.includes(productSearch))).map(p => (
                      <div 
                        key={p.id} 
                        className="p-2 text-xs hover:bg-slate-100 cursor-pointer border-b last:border-0"
                        onClick={() => {
                          setSelectedProduct(p.id);
                          setProductSearch(p.name);
                          setConversionFactor(p.conversionFactor || 1);
                          setShowProductSuggestions(false);
                        }}
                      >
                        {p.cbaCode ? `[${p.cbaCode}] ` : ''}{p.name}
                      </div>
                    ))}
                    {products.filter(p => p.name.toLowerCase().includes(productSearch.toLowerCase()) || (p.cbaCode && p.cbaCode.includes(productSearch))).length === 0 && (
              ...
              <div className="grid grid-cols-3 gap-2 mb-2">
                <input type="number" className="border p-1 rounded text-xs" placeholder="Qtd" value={qty} onChange={e => setQty(e.target.value)} />
                <input type="number" className="border p-1 rounded text-xs" placeholder="Múltiplo" value={conversionFactor} onChange={e => setConversionFactor(e.target.value)} />
                <input type="number" className="border p-1 rounded text-xs" placeholder="Custo Unit." value={unitCost} onChange={e => setUnitCost(e.target.value)} />
              </div>
              <button onClick={handleAddItem} className="w-full bg-indigo-100 text-indigo-700 py-1 rounded text-xs font-bold hover:bg-indigo-200">+ Adicionar Item</button>
              
              <div className="mt-2 space-y-1 max-h-32 overflow-y-auto">
                {items.map((item, i) => (
                  <div key={i} className="flex justify-between text-xs border-b pb-1">
                    <span>{item.originalQty || item.qty}x {item.conversionFactor > 1 ? `(Cx ${item.conversionFactor})` : ''} {item.name}</span>
                    <div className="flex items-center gap-2">
                      <span>{formatCurrency(item.total)}</span>
                      <button onClick={() => handleRemoveItem(i)} className="text-red-500"><X size={12}/></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <button onClick={handleSave} className="w-full bg-slate-800 text-white py-3 rounded font-bold hover:bg-slate-700">{editingId ? 'Atualizar' : 'Salvar'}</button>
        </div>
      </div>

      {/* Lista de Histórico */}
      <div className="lg:col-span-2 bg-white rounded border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        <div className="p-4 border-b bg-slate-50 font-bold text-slate-700">Histórico de Notas e Gastos</div>
        <div className="flex-1 overflow-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500 uppercase text-xs font-semibold">
              <tr><th className="p-4">Data</th><th className="p-4">Tipo</th><th className="p-4">Categoria</th><th className="p-4">Descrição</th><th className="p-4 text-right">Valor</th><th className="p-4 text-right">Ações</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {[...transactions].reverse().map(t => (
                <tr key={t.id} className="hover:bg-slate-50">
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded text-xs font-bold ${t.type === 'entry' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                  ...
                  <td className="p-4">
                    ...
                    {t.items.length > 0 && <div className="text-xs text-slate-400">{t.items.length} itens movimentados</div>}
                  </td>
                  <td className={`p-4 text-right font-bold ${t.type === 'entry' ? 'text-emerald-600' : 'text-red-600'}`}>
                    {t.type === 'entry' ? '+' : '-'}{formatCurrency(t.value)}
                  </td>
                  <td className="p-4 text-right">
                    <button onClick={() => handleEdit(t)} className="text-slate-400 hover:text-indigo-600 mr-2"><Edit size={16}/></button>
                    <button onClick={() => onDeleteTransaction(t.id)} className="text-slate-400 hover:text-red-600"><Trash2 size={16}/></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {activeTab === 'categories' && (
        <div className="space-y-6">
          <div className="bg-white p-6 rounded border border-slate-200 shadow-sm flex gap-4 items-end">
            <div className="flex-1">
              <label className="block text-xs font-bold text-slate-700 mb-1">Nome da Nova Categoria</label>
              <input className="w-full border p-2 rounded text-sm" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} placeholder="Ex: Manutenção, Impostos..." />
            </div>
            <button onClick={handleAddCategory} className="bg-slate-800 text-white px-4 py-2 rounded text-sm font-medium hover:bg-slate-700 flex items-center gap-2 h-10">
          ...
          <div className="bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-500 uppercase text-xs font-semibold">
                <tr><th className="p-4">Nome da Categoria</th><th className="p-4 text-right">Ação</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {transactionCategories.filter(c => c.type !== 'system').map(c => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    <td className="p-4 font-medium text-slate-800">{c.name}</td>
                    <td className="p-4 text-right"><button onClick={() => handleDeleteCategory(c.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={18}/></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
