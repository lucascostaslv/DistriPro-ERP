...
const CashClosingManager = ({ showNotification }) => {
  ...
  useEffect(() => {
    ...
    const unsubAccs = tenantDB.firestore.subscribe(
      "bank_accounts",
      setBankAccounts,
    );
    const unsubTxns = tenantDB.firestore.subscribe(
      "account_transactions",
      setBankTxns,
    );

    const unsubClosings = tenantDB.firestore.subscribe(
      "cash_closings",
      (data) => {
        setClosings(data);
        setLoading(false);
  ...
  const calendarGrid = useMemo(() => {
    ...
    for (let i = 0; i < firstDayOfWeek; i++)
      grid.push({ type: "padding", id: `pad-${i}` });
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (dayOfWeek === 6) {
        ...
        mon.setDate(dateObj.getDate() + 2);
        const sun = new Date(dateObj);
        sun.setDate(dateObj.getDate() + 1);
        ...
        const sat = new Date(dateObj);
        sat.setDate(dateObj.getDate() - 2);
        const sun = new Date(dateObj);
        sun.setDate(dateObj.getDate() - 1);
        if (sat.getMonth() === month)
          periodDatesIncluded = [
            sat.toISOString().split("T")[0],
            sun.toISOString().split("T")[0],
            dateStr,
      ...
      const existingClosing = closings.find(
        (c) => c.periodId === belongsToPeriodId,
      );
      ...
      if (existingClosing) {
        const periodExpenses = expenses.filter(
          (e) =>
            periodDatesIncluded.includes(
              safeGetDate(e.date) || safeGetDate(e.createdAt),
            ) && e.status === "PAGO",
        );
        const periodTxns = bankTxns.filter((t) =>
          periodDatesIncluded.includes(
            safeGetDate(t.date) || safeGetDate(t.createdAt),
          ),
        );

        const auditableExpenses = periodExpenses.filter(
          (e) => e.accountId === "" || !e.accountId,
        );
        const auditableTxnIds = [
          ...auditableExpenses.map((e) => e.id),
          ...periodTxns.map((t) => t.id),
        ];

        const acknowledgedTxns = new Set(
          existingClosing.acknowledgedTxns || [],
        );
        hasMissingChecks = auditableTxnIds.some(
          (id) => !acknowledgedTxns.has(id),
        );
      }
      ...
      if (existingClosing) {
        if (existingClosing.status === "INFERIOR_PENDENTE") {
          ...
          existingClosing.status === "PENDENTE_CONCILIACAO" ||
          hasMissingChecks
        ) {
      ...
      grid.push({
        ...
        day,
        dateStr,
        periodId: belongsToPeriodId,
        datesIncluded: periodDatesIncluded,
        status: visualStatus,
        isWeekend: dayOfWeek === 0 || dayOfWeek === 6,
        isGroupedDay,
        closingData: existingClosing || null,
      });
    }
    return grid;
  ...
  const selectedPeriodData = useMemo(() => {
    ...
    const periodSales = sales.filter((s) =>
      datesIncluded.includes(safeGetDate(s.date) || safeGetDate(s.createdAt)),
    );
    const periodExpenses = expenses.filter(
      (e) =>
        datesIncluded.includes(
          safeGetDate(e.date) || safeGetDate(e.createdAt),
        ) && e.status === "PAGO",
    );

    const salesDinheiro = periodSales.reduce(
      (acc, curr) =>
        acc + (curr.paymentMethod === "DINHEIRO" ? Number(curr.total) || 0 : 0),
      0,
    );
    const salesOutros = periodSales.reduce(
      (acc, curr) =>
        acc + (curr.paymentMethod !== "DINHEIRO" ? Number(curr.total) || 0 : 0),
      0,
    );
    const expensesGaveta = periodExpenses
      .filter((e) => e.accountId === "" || !e.accountId)
      .reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);

    const expectedCash = salesDinheiro - expensesGaveta;

    const periodTxns = bankTxns.filter((t) =>
      datesIncluded.includes(safeGetDate(t.date) || safeGetDate(t.createdAt)),
    );
    const groupedBanks = {};
    ...
    periodTxns.forEach((t) => {
      if (!groupedBanks[t.accountId]) {
        ...
        groupedBanks[t.accountId] = {
          name: accInfo ? accInfo.name : "Conta Não Identificada",
          in: [],
          out: [],
          totalIn: 0,
          totalOut: 0,
      ...
      if (t.type === "IN") {
        groupedBanks[t.accountId].in.push(t);
        groupedBanks[t.accountId].totalIn += Number(t.amount);
      } else {
        groupedBanks[t.accountId].out.push(t);
        groupedBanks[t.accountId].totalOut += Number(t.amount);
      }
    });
    ...
    return {
      ...
      expensesGaveta,
      expectedCash,
      banks: Object.values(groupedBanks),
      auditableExpenses,
      auditableTxnIds,
  ...
  const pendingAlerts = useMemo(() => {
    const pendings = closings.filter(
      (c) =>
        c.status === "INFERIOR_PENDENTE" || c.status === "PENDENTE_CONCILIACAO",
    );
  ...
  const handleSelectDay = (dayInfo) => {
    ...
    setActualCashInput(
      dayInfo.closingData?.actualCash
        ? String(dayInfo.closingData.actualCash).replace(".", ",")
        : "",
    );
  ...
  const handleSaveClosing = async () => {
    if (!actualCashInput && actualCashInput !== "0")
      return showNotification("Informe o valor real em caixa.", "error");
    ...
    const missingChecks = selectedPeriodData.auditableTxnIds.some(
      (id) => !checkedTxns.has(id),
    );
    ...
    if (diff > 0.05) {
      if (!obs)
        return showNotification(
          "LUCRO SUPERIOR: Para sobras no caixa, é obrigatório inserir uma justificativa.",
          "error",
        );
      finalStatus = missingChecks ? "PENDENTE_CONCILIACAO" : "SUPERIOR_OK";
      showNotification(
        missingChecks
          ? "Sobra justificada, mas há transações não reconhecidas (Amarelo)."
          : "Caixa fechado com sobra justificada!",
        missingChecks ? "warning" : "success",
      );
    } else if (diff < -0.05) {
      finalStatus = "INFERIOR_PENDENTE";
      showNotification(
        "LUCRO INFERIOR: Quebra de caixa detectada! O dia ficará travado em Vermelho.",
        "error",
      );
    } else {
      ...
      showNotification(
        missingChecks
          ? "Valores batem, mas há transações não reconhecidas (Amarelo)."
          : "Caixa batido e conciliado perfeitamente!",
        missingChecks ? "warning" : "success",
      );
    }
    ...
    try {
      const payload = {
        periodId: selectedPeriodClosing.periodId,
      ...
      if (selectedPeriodClosing.closingData)
        await tenantDB.firestore.update(
          "cash_closings",
          selectedPeriodClosing.closingData.id,
          payload,
        );
      else await tenantDB.firestore.add("cash_closings", payload);
      setSelectedPeriodClosing(null);
    } catch (e) {
      showNotification("Erro ao registrar fechamento.", "error");
    }
  };
  ...
  if (loading)
    return (
      <div className="p-10 flex flex-col items-center text-slate-400">
        <Clock className="animate-spin mb-2" /> Sincronizando dados bancários...
      </div>
    );
  const monthNames = [
    "Janeiro",
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div
            className={`p-2.5 rounded-lg ${pendingAlerts ? (pendingAlerts.qteVermelho > 0 ? "bg-red-50 text-red-600" : "bg-amber-50 text-amber-600") : "bg-slate-50 text-slate-800"}`}
          >
          ...
          <div>
            ...
            {pendingAlerts && (
              <p
                className={`text-[10px] font-bold uppercase tracking-wider mt-1 ${pendingAlerts.qteVermelho > 0 ? "text-red-600" : "text-amber-600"}`}
              >
                {pendingAlerts.qteVermelho} Quebras (Vermelho) e{" "}
                {pendingAlerts.qteAmarelo} Não Reconhecidos (Amarelo) em:{" "}
                {pendingAlerts.monthsList}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center bg-slate-50 rounded-lg p-1 border border-slate-200">
          <button
            ...
            className="p-1.5 rounded hover:bg-white text-slate-400 hover:text-slate-800"
          >
            <ChevronLeft size={16} />
          </button>
      ...
      <div className="bg-white p-5 rounded-xl border shadow-sm">
        <div className="grid grid-cols-7 gap-2 mb-2 text-center">
          {["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"].map((day, i) => (
            <div
              key={day}
              className={`text-[10px] font-bold uppercase ${i === 0 || i === 6 ? "text-red-400" : "text-slate-400"}`}
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 gap-2">
          {calendarGrid.map((item) => {
            if (item.type === "padding")
              return <div key={item.id} className="aspect-square"></div>;
            ...
            if (item.status === "OPEN") {
              statusClasses =
                "bg-white text-slate-700 border-slate-200 hover:border-indigo-400 cursor-pointer shadow-sm";
              ...
              statusClasses =
                "bg-red-50 text-red-800 border-red-200 cursor-pointer animate-pulse-slow";
              badge = <AlertTriangle size={12} className="text-red-500" />;
            } else if (item.status === "PENDING_YELLOW") {
              statusClasses =
                "bg-amber-50 text-amber-900 border-amber-200 cursor-pointer shadow-sm";
            ...
            return (
              <div
                ...
                className={`relative aspect-square rounded-lg border p-2 flex flex-col justify-between transition-all ${statusClasses}`}
              >
                ...
                {item.isGroupedDay && item.status !== "FUTURE" && (
                  <span className="text-[8px] bg-slate-900/5 text-slate-500 px-1 py-0.5 rounded font-bold uppercase w-full text-center mt-1">
                    Fim de Sem.
                  </span>
                )}
                ...
                {item.closingData && item.status !== "FUTURE" && (
                  <span
                    className={`text-[9px] font-bold mt-auto ${item.closingData.difference > 0 ? "text-blue-600" : item.closingData.difference < 0 ? "text-red-600" : "text-emerald-600"}`}
                  >
                    {item.closingData.difference !== 0
                      ? item.closingData.difference > 0
                        ? "+"
                        : "-"
                      : ""}
                    {formatCurrency(Math.abs(item.closingData.difference))}
                  </span>
                )}
              </div>
            );
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white w-full max-w-3xl rounded-xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b flex justify-between items-center bg-white shrink-0">
              <div>
                <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                  ...
                  {selectedPeriodClosing.isGroupedDay
                    ? `Fechamento (Sáb - Seg)`
                    : `Fechamento Diário`}
                </h3>
                <p className="text-[11px] text-slate-400 uppercase font-bold tracking-wider mt-0.5">
                  Ref:{" "}
                  {selectedPeriodClosing.datesIncluded
                    .map((d) => safeGetDate(d).split("-").reverse().join("/"))
                    .join(" a ")}
                </p>
              </div>
              <button
                onClick={() => setSelectedPeriodClosing(null)}
            ...
            <div className="flex px-4 pt-2 border-b bg-slate-50 shrink-0 gap-4">
              <button
                onClick={() => setModalTab("gaveta")}
                className={`pb-3 text-sm font-bold border-b-2 flex items-center gap-2 px-2 transition-colors ${modalTab === "gaveta" ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-700"}`}
              >
                <Wallet size={16} /> Gaveta (Dinheiro)
              </button>
              <button
                onClick={() => setModalTab("banco")}
                className={`pb-3 text-sm font-bold border-b-2 flex items-center gap-2 px-2 transition-colors ${modalTab === "banco" ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-700"}`}
              >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="bg-white p-5 rounded-xl border shadow-sm border-slate-200 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-extrabold text-slate-800">
                          {formatCurrency(selectedPeriodData.expectedCash)}
                        </p>
                      </div>
                      <div className="mt-4 space-y-2 pt-4 border-t border-slate-100 text-sm">
                        <div className="flex justify-between text-emerald-700">
                          ...
                          <span>
                            {formatCurrency(selectedPeriodData.salesDinheiro)}
                          </span>
                        </div>
                        <div className="flex justify-between text-red-600">
                          <span className="flex items-center gap-1.5 font-bold">
                            <TrendingDown size={14} /> Saídas Físicas
                          </span>{" "}
                          <span>
                            -{formatCurrency(selectedPeriodData.expensesGaveta)}
                          </span>
                        </div>
                        <div className="flex justify-between text-slate-400 text-xs pt-1">
                          <span className="italic">
                            Vendas Cartão/Pix (Ignoradas)
                          </span>{" "}
                          <span>
                            {formatCurrency(selectedPeriodData.salesOutros)}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-xl border shadow-sm border-indigo-100 flex flex-col">
                      <label className="text-[10px] font-bold text-slate-500 uppercase mb-2 flex items-center gap-1">
                        <DollarSign size={12} /> Dinheiro Contado
                      </label>
                      <input
                        type="text"
                        placeholder="0,00"
                    ...
                    <div className="bg-white border rounded-xl overflow-hidden shadow-sm mt-4">
                      <div className="bg-slate-100 p-3 px-4 flex justify-between items-center border-b">
                        <h4 className="font-bold text-slate-800 text-xs uppercase">
                          Saídas na Gaveta (Reconhecimento)
                        </h4>
                        <div className="flex gap-2">
                          <button
                            onClick={() =>
                              handleSelectAll(
                                selectedPeriodData.auditableExpenses.map(
                                  (e) => e.id,
                                ),
                                true,
                              )
                            }
                            className="text-[10px] bg-white border px-2 py-1 rounded text-slate-600 hover:bg-slate-50 font-bold"
                          >
                            Marcar Todos
                          </button>
                          <button
                      ...
                      <table className="w-full text-left text-xs">
                        <tbody className="divide-y divide-slate-50">
                          {selectedPeriodData.auditableExpenses.map((exp) => (
                            <tr
                              key={exp.id}
                              className={`hover:bg-slate-50 transition-colors cursor-pointer ${checkedTxns.has(exp.id) ? "bg-emerald-50/30" : ""}`}
                              onClick={() => toggleCheckbox(exp.id)}
                            >
                              <td className="p-3 w-8 text-center">
                                <input
                                  type="checkbox"
                                  checked={checkedTxns.has(exp.id)}
                                  readOnly
                ...
                <div className="space-y-4 animate-in fade-in">
                  {selectedPeriodData.banks.length === 0 ? (
                    <div className="text-center p-8 text-slate-400 border border-dashed rounded-xl bg-white">
                      Nenhuma movimentação bancária registrada neste período.
                    </div>
                  ) : (
                    selectedPeriodData.banks.map((bank, idx) => {
                      const allBankTxnIds = [...bank.in, ...bank.out].map(
                        (t) => t.id,
                      );
                      return (
                        <div
                            ...
                            <div className="flex gap-3 items-center">
                              <div className="flex gap-3 text-xs font-bold mr-4">
                                <span className="text-emerald-600">
                                  IN: {formatCurrency(bank.totalIn)}
                                </span>
                                <span className="text-red-500">
                                  OUT: {formatCurrency(bank.totalOut)}
                                </span>
                              </div>
                              <button
                                onClick={() =>
                                  handleSelectAll(allBankTxnIds, true)
                                }
                                className="text-[10px] bg-white border px-2 py-1 rounded text-slate-600 hover:bg-slate-50 font-bold"
                              >
                                Marcar Todos
                              ...
                              <button
                                onClick={() =>
                                  handleSelectAll(allBankTxnIds, false)
                                }
                                className="text-[10px] bg-white border px-2 py-1 rounded text-slate-600 hover:bg-slate-50 font-bold"
                              >
                                Desmarcar
                              ...
                              <tbody className="divide-y divide-slate-50">
                                {[...bank.in, ...bank.out]
                                  ...
                                  .map((txn, i) => (
                                    <tr
                                      key={txn.id}
                                      className={`hover:bg-slate-50 transition-colors cursor-pointer ${checkedTxns.has(txn.id) ? "bg-emerald-50/30" : ""}`}
                                      onClick={() => toggleCheckbox(txn.id)}
                                    >
                                      <td className="p-3 w-8 text-center">
                                        <input
                                          type="checkbox"
                                          checked={checkedTxns.has(txn.id)}
                                          readOnly
                                          className="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"
                                        />
                                      </td>
                                      ...
                                      <td
                                        className={`p-3 text-right font-bold ${txn.type === "IN" ? "text-emerald-600" : "text-red-500"}`}
                                      >
                                        {txn.type === "IN" ? "+" : "-"}
            ...
            <div className="p-4 bg-white border-t flex justify-end gap-3 shrink-0">
              <button
                onClick={() => setSelectedPeriodClosing(null)}
                className="px-5 py-2.5 rounded-lg text-slate-500 font-bold hover:bg-slate-100 text-sm"
              >
                Cancelar
              </button>
              <button
