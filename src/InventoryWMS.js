...
const StockCard = ({
  ...
  return (
    <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden group">
      ...
      <div
        className={`absolute top-0 left-0 p-1 px-2 rounded-br text-[9px] font-bold uppercase ${isPack ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
      >
      ...
      <div className="mt-4">
        <div className="flex justify-between items-start pr-4">
          <h3
            className="font-bold text-slate-800 line-clamp-2 min-h-[40px] text-sm leading-tight cursor-pointer hover:text-indigo-600"
            onClick={() => onOpenHistory(product)}
          >
            {product.name}
          </h3>
        ...
        <div className="mt-1 space-y-1">
          <div className="text-xs text-slate-500 font-mono flex items-center gap-1">
            <span className="bg-slate-100 px-1 rounded">
              {product.barcode || product.cbaCode || "S/ CÓDIGO"}
            </span>
          </div>
          {isPack && (
            <div className="text-[10px] text-purple-600 flex items-center gap-1 bg-purple-50 p-1 rounded w-fit">
              <Boxes size={10} /> Contém{" "}
              {product.packQuantity || product.conversionFactor}x{" "}
              {getParentName(product.parentId)}
            </div>
          )}
        </div>
      ...
      <div className="mt-3 pt-2 border-t border-slate-100">
        <div className="flex justify-between items-end mb-2">
          ...
          <span
            className={`text-xl font-bold ${displayStock < (product.minStock || 5) ? "text-red-500" : "text-slate-800"}`}
          >
            {displayStock}{" "}
        ...
        <div className="flex gap-1">
          ...
          <button
            onClick={() => {
              if (qtyInput) handleQuickStock(Number(qtyInput));
            }}
          ...
          <button
            onClick={() => {
              if (qtyInput) handleQuickStock(Number(qtyInput) * -1);
            }}
...
const InventoryWMS = ({
  ...
  useEffect(() => {
    const loadAuxData = async () => {
      ...
      const { data: suppliersData } =
        await tenantDB.supabase.query("fiscal_clients");
  ...
  const addSupplierLink = async () => {
    if (!newSupplierLink.name)
      return showNotification("Nome do fornecedor obrigatório", "error");
    ...
    try {
      if (matchSupplier) {
        ...
        if (!storeConfig?.id)
          return showNotification("Erro: Loja não identificada.", "error");
        ...
        const { data, error } = await tenantDB.supabase.insertAndReturn(
          "fiscal_clients",
          newClientPayload,
        );
        ...
        showNotification(
          `Fornecedor "${supplierNameUpper}" cadastrado automaticamente!`,
          "success",
        );
      }
      ...
      setCurrentProduct((prev) => {
        const currentHistory = Array.isArray(prev.suppliersHistory)
          ? prev.suppliersHistory
          : [];
        // Remove duplicados se houver (atualiza o existente)
        const otherSuppliers = currentHistory.filter(
          (s) => String(s.supplierId) !== String(realSupplierId),
        );
      ...
      showNotification(
        "Erro ao salvar novo fornecedor: " + error.message,
        "error",
      );
    }
  };
  ...
  const removeSupplierLink = (idx) => {
    const currentHistory = Array.isArray(currentProduct.suppliersHistory)
    ...
    try {
      ...
      if (!finalTaxProfileId && currentProduct.parentId) {
        const parentProduct = products.find(
          (p) => p.id === currentProduct.parentId,
        );
      ...
      const productData = {
        ...
        conversionFactor:
          Number(
            currentProduct.conversionFactor || currentProduct.packQuantity,
          ) || 1,
        parentId: currentProduct.parentId || "", // Evita undefined se não for caixa

        // Timestamp de atualização
        last_updated: tenantDB.firestore.utils.serverTimestamp(),
      };

      let mainDocRef;
      // 1. Salva Principal
      if (currentProduct.id) {
        await tenantDB.firestore.update(
          "products",
          currentProduct.id,
          productData,
        );
      ...
      if (createPackMode && mainDocRef.id) {
        const packData = {
          ...
          name: packFormData.name
            ? packFormData.name.toUpperCase()
            : `CX ${currentProduct.name}`,
          ...
          unit: "CX",
          created_at: tenantDB.firestore.utils.serverTimestamp(),
        };
  ...
  const processStockUpdate = async (product, qtyChange, reason = "") => {
    try {
      ...
      showNotification(
        qtyChange > 0 ? "Estoque adicionado!" : "Baixa realizada.",
        "success",
      );
  ...
  const fetchAuditTemplates = async () => {
    ...
    try {
      const storeId = String(storeConfig.id);
      const templates = await tenantDB.firestore.getAll(
        "inventory_audit_templates",
      );
  ...
  const handleSaveTemplate = async () => {
    if (!storeConfig?.id)
      return showNotification("Loja não identificada.", "error");
    if (!newTemplateName.trim() || auditSelection.length === 0) {
      return showNotification(
        "Digite um nome e selecione produtos para salvar.",
        "error",
      );
    }
    try {
  ...
  const handleLoadTemplate = (e) => {
    ...
    if (template) {
      const productsToLoad = products.filter((p) =>
  ...
  const handleGenerateAuditPDF = () => {
    if (auditSelection.length === 0)
      return showNotification("Selecione ao menos um produto.", "error");
  ...
  const handleCopyQuote = (supplierName, items) => {
    const text =
      `*COTAÇÃO - ${supplierName}*\n\n` +
      items
        .map((i) => `- ${i.missing}x ${i.name} (${i.stock} em estoque)`)
        .join("\n") +
  ...
  const handleUpdateQuoteQty = (id, val) => {
    setQuotationItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, buyQty: Number(val) } : i)),
    );
  };
            ...
            <div className="flex gap-2 ml-4">
              {activeTab === "management" && (
                <button
                  ...
                  className="bg-indigo-50 text-indigo-700 px-4 py-2 rounded font-bold hover:bg-indigo-100 flex items-center gap-2 border border-indigo-200"
                >
              ...
              {activeTab === "management" && (
                <button
                  ...
                  className="bg-slate-800 text-white px-4 py-2 rounded font-bold hover:bg-slate-700 flex items-center gap-2"
                >
              ...
              {activeTab === "management" && (
                <button
                  ...
                  className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded font-bold hover:bg-emerald-100 flex items-center gap-2 border border-emerald-200"
                >
              ...
              <div className="p-3 border-t bg-amber-50 space-y-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (
                      !window.confirm(
                        `Confirma a baixa de ${saidaCart.length} item(ns)?`,
                      )
                    )
                      return;
                    try {
                ...
                >
                  <td className="p-4">
                    ...
                    <div className="text-xs text-slate-500">
                      {prod.barcode || prod.cbaCode}
                    </div>
                  </td>
                  <td className="p-4 text-right font-bold text-slate-700">
                    {masks.currency(prod.price)}
                  </td>
                  <td className="p-4 text-right font-bold text-emerald-600">
                    {prod.wholesalePrice > 0
                      ? masks.currency(prod.wholesalePrice)
                      : "-"}
                  </td>
                  ...
                  <td className="p-4 text-center">
                    <span
                      className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${prod.itemType === "pack" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {paginatedDeadStock.map((prod) => {
                ...
                return (
                  <div
                    key={prod.id}
                    className="bg-white p-4 rounded border border-red-200 shadow-sm relative overflow-hidden"
                  >
                    <div
                      className={`absolute top-0 right-0 text-xs font-bold px-2 py-1 rounded-bl ${hasSale ? "bg-red-100 text-red-700" : "bg-slate-700 text-white"}`}
                    >
                      {hasSale ? `${days} dias sem venda` : "Nunca Vendido"}
                    </div>

                    <h3 className="font-bold text-slate-800 pr-20 truncate">
                      {prod.name}
                    </h3>
          ...
          <div className="bg-white w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="space-y-6 animate-in fade-in">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer bg-orange-100 p-2 rounded border border-orange-200">
                          <input
                            ...
                            onChange={(e) =>
                              setCurrentProduct((prev) => ({
                                ...prev,
                                isCheckoutEnabled: e.target.checked,
                              }))
                            }
                          ...
                          <div>
                            ...
                            <span className="block text-[10px] text-orange-600 leading-tight">
                              Marque para itens como <strong>Sacolas</strong>{" "}
                              que devem aparecer no PDV.
                            </span>
                          </div>
                        </label>
                      </div>
                    ...
                    <div className="md:col-span-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setCurrentProduct({
                            ...currentProduct,
                            barcode: masks.ean(e.target.value),
                          })
                        }
                    ...
                    <div className="md:col-span-12">
                      <label className="text-[10px] font-bold text-slate-500 uppercase flex items-center gap-1 mb-1">
                        ...
                        <span className="text-slate-300 font-normal normal-case">
                          (promoções, embalagens alternativas)
                        </span>
                      </label>
                      <div className="flex flex-wrap gap-2 mb-2">
                        {(currentProduct.extraBarcodes || []).map((bc, idx) => (
                          <span
                            key={idx}
                            className="flex items-center gap-1 bg-slate-100 border border-slate-200 px-2 py-1 rounded text-xs font-mono text-slate-700"
                          >
                            {bc}
                            <button
                              type="button"
                              onClick={() =>
                                setCurrentProduct((prev) => ({
                                  ...prev,
                                  extraBarcodes: prev.extraBarcodes.filter(
                                    (_, i) => i !== idx,
                                  ),
                                }))
                              }
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter" && e.target.value.trim()) {
                              ...
                              if (
                                !(currentProduct.extraBarcodes || []).includes(
                                  val,
                                )
                              ) {
                                setCurrentProduct((prev) => ({
                        ...
                        <button
                          ...
                          onClick={() => {
                            const input =
                              document.getElementById("extraBarcodeInput");
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-2">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleCostCalculation("costPrice", e.target.value)
                          }
                          placeholder="0.00"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleCostCalculation("price", e.target.value)
                          }
                          placeholder="0.00"
                        />
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input
                        type="number"
                        className="w-full border p-2 rounded text-sm bg-amber-50"
                    ...
                    <div className="md:col-span-12 mt-2 pt-4 border-t border-slate-200">
                      <h4 className="font-bold text-slate-700 mb-3 flex items-center gap-2 text-sm">
                        ...
                        <span className="text-[10px] font-normal text-slate-400">
                          (opcional — preencha se este produto é vendido por
                          dose no PDV)
                        </span>
                      </h4>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setCurrentProduct({
                                ...currentProduct,
                                dosePrice: e.target.value,
                              })
                            }
                          />
                        </div>
                        {currentProduct.bottleVolumeMl > 0 &&
                          currentProduct.doseVolumeMl > 0 && (
                            <div className="col-span-3 bg-purple-50 border border-purple-200 rounded p-2 text-xs text-purple-700 flex items-center gap-2">
                              <Wine size={14} />
                              Esta garrafa rende aproximadamente{" "}
                              <strong>
                                {Math.floor(
                                  currentProduct.bottleVolumeMl /
                                    currentProduct.doseVolumeMl,
                                )}{" "}
                                doses
                              </strong>{" "}
                              de {currentProduct.doseVolumeMl}ml cada.
                            </div>
                          )}
                      </div>
                    </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-200 animate-in fade-in">
                    <h4 className="font-bold text-slate-700 mb-3 flex items-center gap-2">
                      <Boxes size={18} className="text-indigo-600" /> Gestão de
                      Fardos/Caixas
                    </h4>
                    ...
                    {currentProduct.itemType === "unit" && (
                      <div className="bg-slate-50 p-4 rounded border border-slate-200">
                        <p className="text-xs text-slate-500 mb-3 font-bold">
                          Vincular ou Criar Caixa/Fardo para esta unidade:
                        </p>
                        ...
                        <div className="flex flex-col gap-3">
                              ...
                              {products
                                ...
                                .map((p) => (
                                  <option key={p.id} value={p.id}>
                                    {p.name} (Fator:{" "}
                                    {p.conversionFactor || p.packQuantity})
                                  </option>
                                ))}
                            </select>
                          </div>
                          ...
                          {!currentProduct.linkedPackId && (
                            <div
                              className={`mt-2 p-4 rounded border transition-all ${createPackMode ? "bg-white border-indigo-200 shadow-md" : "bg-slate-100 border-dashed border-slate-300"}`}
                            >
                              <label className="flex items-center gap-2 cursor-pointer mb-2">
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (checked) {
                                      ...
                                      setPackFormData({
                                        ...
                                        profitMargin:
                                          currentProduct.profitMargin, // Herda Margem Sugerida
                                ...
                                <span className="font-bold text-slate-700 text-sm">
                                  Não existe? Cadastrar Caixa Completa
                                </span>
                              </label>
                              ...
                              {createPackMode && (
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 animate-in slide-in-from-top-2 mt-3">
                                  ...
                                  <div className="md:col-span-8">
                                    ...
                                    <input
                                      className="w-full border p-2 rounded text-sm uppercase font-bold"
                                  ...
                                  <div className="md:col-span-3">
                                    ...
                                    <input
                                      type="number"
                                      className="w-full border p-2 rounded text-sm font-bold bg-indigo-50"
                                      ...
                                      onChange={(e) => {
                                        ...
                                        const margin =
                                          Number(packFormData.profitMargin) ||
                                          0;
                                        setPackFormData({
                                          ...packFormData,
                                          quantity: q,
                                  ...
                                  <div className="md:col-span-5">
                                    ...
                                    <input
                                      className="w-full border p-2 rounded text-sm bg-slate-100 text-slate-500"
                                      readOnly
                                  ...
                                  <div className="md:col-span-3">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        const c = Number(
                                          packFormData.costPrice,
                                        );
                                        setPackFormData({
                                          ...packFormData,
                                          profitMargin: m,
                                          price: (c * (1 + m / 100)).toFixed(2),
                                        });
                                      }}
                                    />
                                  </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <input
                              type="number"
                              className="w-full border p-2 rounded text-sm"
                              value={currentProduct.packQuantity}
                              onChange={(e) =>
                                setCurrentProduct({
                                  ...currentProduct,
                ...
                <div className="space-y-6 animate-in fade-in">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-700 mb-2 flex items-center gap-2">
                      <Truck size={18} /> Fornecedores Vinculados
                    </h4>
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {currentProduct.suppliersHistory?.length > 0 ? (
                            ...
                            [...currentProduct.suppliersHistory]
                              .sort(
                                (a, b) =>
                                  new Date(b.lastPurchaseDate || 0) -
                                  new Date(a.lastPurchaseDate || 0),
                              )
                              .map((sup, idx, arr) => {
                                // --- CÁLCULO BLINDADO ---
                                ...
                                const safeNum = (val) => {
                                  ...
                                  const n = parseFloat(
                                    String(val).replace(",", "."),
                                  ); // Troca vírgula por ponto por segurança
                                  return isNaN(n) ? 0 : n;
                                };
                                ...
                                const prevPrice = prevItem
                                  ? safeNum(prevItem.lastCost)
                                  : currentPrice;

                                const diff = currentPrice - prevPrice;
                                // Evita divisão por zero
                                const pct =
                                  prevPrice > 0 ? (diff / prevPrice) * 100 : 0;

                                // Para Debug no Console (F12) se precisar
                                console.log(
                                  `Linha ${idx}: Atual=${currentPrice} | Anterior=${prevPrice} | Diff=${diff}`,
                                );
                              ...
                              >
                                <div className="flex flex-col items-center gap-2">
                                  ...
                                  <span className="text-xs">
                                    Nenhum histórico de compra registrado.
                                  </span>
                                </div>
                              </td>
                            </tr>
              ...
              {editTab === "taxes" &&
                (() => {
                  const profile = taxProfiles.find(
                    (tp) =>
                      String(tp.id) === String(currentProduct.taxProfileId),
                  );
                  const taxes = profile
                    ? calculateItemTaxes(
                        {
                    ...
                    <div className="space-y-4 animate-in fade-in p-2">
                      {!profile ? (
                        <div className="bg-amber-50 border border-amber-200 p-4 rounded text-amber-700 text-sm flex items-center gap-2">
                          ...
                          Selecione um Perfil Tributário na aba "Dados Gerais"
                          para visualizar os impostos.
                        </div>
                      ) : (
                        <>
                          ...
                          <table className="w-full text-sm border-collapse">
                            ...
                            <tfoot>
                              <tr className="bg-slate-50 font-bold">
                                ...
                                >
                                  Total Impostos sobre Preço de Venda:
                                </td>
            ...
            <div className="p-4 bg-slate-50 border-t flex justify-end gap-2 shrink-0">
              <button
                ...
                className="px-4 py-2 text-slate-600 hover:bg-slate-200 rounded font-bold text-sm"
              >
                Cancelar
              </button>
              <button
                onClick={handleSave}
                className="px-6 py-2 bg-slate-800 text-white rounded font-bold text-sm hover:bg-slate-900 flex items-center gap-2"
              >
                <Save size={16} />{" "}
                {currentProduct.id
                  ? "Salvar Alterações"
                  : createPackMode
                    ? "Salvar Unidade + Caixa"
                    : "Salvar Produto"}
              </button>
            </div>
          </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl overflow-hidden">
            <div className="bg-slate-800 text-white p-4 flex justify-between items-center">
              <h3 className="font-bold flex items-center gap-2">
                <FileText size={18} /> Ficha Técnica do Produto
              </h3>
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-slate-500 mb-6 font-mono bg-slate-100 w-fit px-2 rounded">
                {selectedHistoryProduct.barcode ||
                  selectedHistoryProduct.cbaCode}
              </p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-slate-50 p-3 rounded border">
                  ...
                  <p className="font-bold text-slate-800 flex items-center gap-2">
                    <Calendar size={14} />{" "}
                    {formatDate(
                      selectedHistoryProduct.created_at ||
                        selectedHistoryProduct.createdAt,
                    )}
                  </p>
                </div>
                <div className="bg-slate-50 p-3 rounded border">
                  ...
                  <p
                    className={`font-bold flex items-center gap-2 ${selectedHistoryProduct.lastSale ? "text-green-600" : "text-slate-400"}`}
                  >
                    <Clock size={14} />{" "}
                    {formatDate(selectedHistoryProduct.lastSale)}
                  </p>
                </div>
                <div className="bg-slate-50 p-3 rounded border">
                  <p className="text-xs text-slate-500 uppercase font-bold">
              ...
              <div className="border-t pt-4">
                ...
                {selectedHistoryProduct.supplierId ? (
                  <div className="flex items-center gap-2 text-sm font-bold text-slate-700 bg-slate-100 p-2 rounded">
                    <Truck size={16} />
                    {suppliers.find(
                      (s) => s.id === selectedHistoryProduct.supplierId,
                    )?.name || "Desconhecido"}
                  </div>
                ) : (
            ...
            <div className="bg-slate-50 p-4 text-center">
              <button
                onClick={() => {
                  ...
                  handleEdit(selectedHistoryProduct);
                }}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-lg w-full max-w-sm shadow-xl">
            <h3 className="font-bold text-red-600 mb-2 flex items-center gap-2">
              <AlertTriangle /> Confirmar Baixa
            </h3>
            <p className="text-sm text-slate-600 mb-4">
              Você está removendo <strong>{Math.abs(lossData.qty)}</strong>{" "}
              itens de <strong>{lossData.product?.name}</strong>.
            </p>
            ...
            <input
              ...
              onChange={(e) =>
                setLossData({ ...lossData, reason: e.target.value })
              }
              autoFocus
            />
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="flex justify-between items-center p-4 border-b bg-slate-50">
              <div>
                <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  <FileText className="text-indigo-600" /> Gerar Auditoria de
                  Estoque
                </h2>
                <p className="text-sm text-slate-500">
                  Selecione os itens e gere a prancheta de contagem em PDF.
                </p>
              </div>
              <button
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="p-4 border-b space-y-3 bg-slate-50">
                  ...
                  <div className="flex gap-2">
                    ...
                    <select
                      ...
                      value={auditFilterGroup}
                      onChange={(e) => setAuditFilterGroup(e.target.value)}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    <label className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={auditShowZero}
                        onChange={(e) => setAuditShowZero(e.target.checked)}
                      />
                ...
                <div className="flex-1 overflow-y-auto p-2">
                  {paginatedAuditList.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        onClick={() => handleToggleAuditProduct(p)}
              ...
              <div className="w-1/2 flex flex-col bg-slate-50/50">
                <div className="p-4 border-b space-y-3 bg-slate-50">
                  <h3 className="font-bold text-slate-700 text-sm">
                    2. Lista de Conferência ({auditSelection.length} itens)
                  </h3>
            ...
            <div className="p-4 border-t bg-white flex justify-between items-center shrink-0">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setIsAuditModalOpen(false)}
                  className="px-5 py-2.5 rounded-lg text-slate-600 font-bold hover:bg-slate-100 text-sm transition-colors"
                >
                  Cancelar
                </button>
                <button
                  onClick={handleGenerateAuditPDF}
                  className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 flex items-center gap-2 text-sm shadow-lg shadow-indigo-200 transition-transform active:scale-95"
                >
